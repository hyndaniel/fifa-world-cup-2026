...
- **结构化结论**经主会话第 5 步 join 进看板 Decision 的 `value` 块(verdict/best_leg/legs)。**`value.verdict` = 决策卡正面那句「一句话」、前端(app.js `dc-verdict`)直接渲染给用户看**,故**必须用上一条同款大白话精简版**——术语就地简注(Poly=聪明钱预测市场/had·hhad=让球玩法)、value·EV% 换算成「每押1元赚/亏X分(毛)」、保留判档含义但用词不堆黑话;**别写** `had三腿对Poly全🔴 / breakeven / 软锚 / steam / -EV` 这类速记体;结论(划算还是别下 → 最不亏腿 → 最该避开腿)先行、单段。它与台账 `> 一句话`、交互首行三处同口径,内容一致但各自成文(verdict 可略紧凑)。
