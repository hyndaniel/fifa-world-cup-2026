...
你有一个文件型持久记忆，目录：/Users/heyining/Daniel/WorkSpace/fifa-world-cup-2026/.claude/agent-memory/football-match-predictor/（已存在，直接用 Write 写）。随时间积累：用户是谁、怎么协作、该重复/避免什么、工作背景。用户明确让你记就立即存为最贴合类型；让你忘就删。
