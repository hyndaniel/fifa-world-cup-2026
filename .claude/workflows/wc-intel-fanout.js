...
//   固化下来,以后每天**只改下面 MATCHES + TODAY + ROUND_CTX 三个常量**,配方结构焊死、不再漂。
//   ⚠️ 别把这句读成「只改 MATCHES」:ROUND_CTX 决定「停赛与黄牌」角度问的是哪一轮的时点,
//      漏改会问出错的事实(2026-07-17 实际发生过,见 ROUND_CTX 处的血泪注释)。
...
//   2. 把这几场填进下面 MATCHES 常量 + 同步改 TODAY 与 ROUND_CTX(改文件,别用 args —— memory wc-fanout-run-gotchas:
...
// 🔴 轮次上下文——**必须跟着 MATCHES 一起改**(否则「停赛与黄牌」角度会拿上一轮的时点去问,
//    问出错的事实)。血泪:2026-07-17 那轮 MATCHES 已换成决赛/季军赛,这里却还写着「今晚是
//    半决赛(SF)…QF 吃到的红牌才停本场 SF」——事实上对决赛/季军赛而言,造成停赛的是 **SF** 的
//    红牌而非 QF。当轮靠 MATCHES 的 ko 文案把结论救了回来,但那是运气不是设计。
//    (既定做法见 675fc3b「MATCHES 换成两场半决赛 + 黄牌规则时点修到 SF」——本变量即为把这行
//     从 ANGLES 里提出来,让它无法再被漏改。)
//    now  = 本轮是哪一轮;prev = 哪一轮场上的红牌/两黄会导致本场停赛(通常 = 上一轮)
const ROUND_CTX = { now: '决赛(FINAL)/季军赛(3rd place)', prev: '半决赛(SF)' }
// ⬆⬆⬆ 每天只改这里(MATCHES + TODAY + ROUND_CTX)⬆⬆⬆

// 6 角度(焊死,对齐 07-03/04/05 验证过的分类法:停赛与黄牌、裁判各自单拎),逐场检索
const ANGLES = [
  { key: '伤停出勤', prompt: '伤病、体能/带伤、出勤状态、是否报销(区分已发生确证 vs 存疑)' },
  { key: '停赛与黄牌', prompt: '停赛名单、直红/双黄罚下、黄牌累计规则(FIFA:淘汰赛 R32/R16/QF 三轮单黄合并累计、不清零,但**单黄累计在 QF 全部打完后已为安全过关者清零一次**,以保证无人因累计黄缺席决赛〈07-10 QF098 轮 3 票证伪坐实,ESPN/NBC/DraftKings 口径一致;勿回退〉。'
      + `**本轮是 ${ROUND_CTX.now}:单黄累计早在 QF 后已清零,不存在「一黄在身、再吃一张即停赛」这回事**——只有「${ROUND_CTX.prev} 场上吃到的直红/同场两黄」这类罚下,才会导致本场停赛;`
      + '逐名点出本场有无确定停赛者(含红牌禁赛未满、FIFA 追加处罚),并明确说明单黄已清零、无累计黄停赛风险。若某源仍称某人「一黄在身将停赛」,标为可疑待证伪)' },
