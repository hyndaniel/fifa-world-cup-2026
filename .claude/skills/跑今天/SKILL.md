...
**为什么(必须懂,不是仪式):** 项目用三方 Brier 跑分卡(`reports/scoring/三方跑分卡.md`)对照
...
1. 取 ET 当下时刻:
   ...
   ```
2. 读进度,看已落盘到哪一天:
   - `reports/agents/wc-score-v1__比分预测.md`——逐日 `## M.D(第 N 比赛日,X 场)` 节,看最后一节是哪天、
...
3. **叙事回填闸(血泪:R32 077-079 曾漏,2026-07-01 才补)。** 机械回填(`match_results` 表 /
   `复盘对错标.md` / Brier 三方跑分卡)由 `tools/backfill_results.py` 经 launchd 每 5 分钟自动重生成,
   不用管、也不会漏。但 `wc-score-v1__比分预测.md` 的"实际"+命中小结、`wc-bet__下注复盘.md` 的
   "赛后"+验证点是**叙事文字**(方向/精确对错判断、母题点评、EV 是否兑现),机械脚本写不出来,
   **只能靠派 agent 补**——除本步外没有任何别的触发点会去查它,漏了不会自愈也不会报错。
   故每次跑本 skill,先查一遍再往下走:
   ```
   sqlite3 .cache/odds_cache.db "select match_key,home_goals,away_goals,ts from match_results order by ts desc limit 10;"
   ```
   把返回的近期场次逐个去两份报告里搜对应卡片——若某场 `match_results` 已有终比分,但报告里仍是
   "实际 _待回填_ / 赛后 *待回填*",说明**已经踢完却没叙事回填**:先并行派 wc-score-v1(回填比分卡片)
   和 wc-bet(回填价值判档表的赛后列),补完这些历史场次,再继续下面判定今天该预测哪几场。
   (顺序:先补昨天的账,再开今天的预测。)
4. 按 ET 当下推出"今天/明天该预测哪几场":
   - 报告里每场都标了 `Xpm ET(= 北京 M.D HH:MM)`。用 ET 当下 + 报告进度,选出
     **尚未落预测、且开球在 ET 今日(或紧邻次日)的场次**。
   - 同组末轮两场同时开球——一起算。
5. 把判定结果(场次清单 + 各场 `zucai_num`(形如 `周四055`)+ 主-客球队名)念给用户确认,
...
3. v1 只落 `v1_predictions` 表 + agents/wc-score-v1__比分预测.md;v2 只读 odds 缓存 + enrich——两套存储不相交。
   三重隔离。wc-odds / wc-bet 是**下游**(综合三方的终点),看 v1/v2 合理,不受红线约束。

**各 agent 自落库(脚本里已指示):** v1 跑 `record_v1` + 回灌 `agents/wc-score-v1__比分预测.md`;v2 跑
`build_v2_prediction → record_v2_prediction`;wc-bet 维护 `reports/agents/wc-bet__下注复盘.md`。所以 workflow
...
```
→ 渲染 `reports/scoring/三方跑分卡.md`(v1 / v2 / 市场基线 三方平均 Brier + 偏离审计 + 归因)。
