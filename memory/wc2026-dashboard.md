...
- 足彩 `tools/collect_zucai.py` → **必须 Mac 住宅 IP** (HK 机房 IP 被 EdgeOne WAF + JA3 拦)。常驻 = **launchd** `~/Library/LaunchAgents/com.wc.collect-zucai.plist` (每 180s `--once`, RunAtLoad, perms 600, 日志 `/tmp/wc_zucai.log`)。POST `/api/ingest/zucai`。
